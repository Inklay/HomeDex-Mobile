...
export async function testAllPokemon (abilities = undefined) {
  ...
  for (let i = start; i < end; i++) {
    ...
    for (let j = 0; j < data.length; j++) {
      ...
      console.assert(data[j].names.find(name => name.language === 'fr').name !== '', `${data[j].names[0].name}, form ${j}: French name empty`)
      console.assert(data[j].names.find(name => name.language === 'es') !== undefined, `${data[j].names[0].name}, form ${j}: No spanish name`)
      console.assert(data[j].names.find(name => name.language === 'es').name !== '', `${data[j].names[0].name}, form ${j}: Spanish name empty`)
      console.assert(data[j].names.find(name => name.language === 'de') !== undefined, `${data[j].names[0].name}, form ${j}: No german name`)
      console.assert(data[j].names.find(name => name.language === 'de').name !== '', `${data[j].names[0].name}, form ${j}: German name empty`)
      console.assert(data[j].names.find(name => name.language === 'ja') !== undefined, `${data[j].names[0].name}, form ${j}: No japanese name`)
      console.assert(data[j].names.find(name => name.language === 'ja').name !== '', `${data[j].names[0].name}, form ${j}: Japanese name empty`)
      console.assert(data[j].names.find(name => name.language === 'ko') !== undefined, `${data[j].names[0].name}, form ${j}: No korean name`)
      console.assert(data[j].names.find(name => name.language === 'ko').name !== '', `${data[j].names[0].name}, form ${j}: Korean name empty`)
      console.assert(data[j].names.find(name => name.language === 'zh-Hant') !== undefined, `${data[j].names[0].name}, form ${j}: No chinese name`)
      console.assert(data[j].names.find(name => name.language === 'zh-Hant').name !== '', `${data[j].names[0].name}, form ${j}: Chinese name empty`)
      // Dex numbers
